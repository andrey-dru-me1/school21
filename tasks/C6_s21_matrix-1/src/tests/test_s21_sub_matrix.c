...
START_TEST(test_complex_sub) {
  ...
  ck_assert_int_eq(s21_sub_matrix(&a, &b, &result), OK);
  matrix_t expected = {
      .matrix = (double *[]){(double[]){0.05, 0.1}, (double[]){0.2, -155.6}},
      .rows = 2,
      .columns = 2};
  ck_assert_int_eq(s21_eq_matrix(&result, &expected), SUCCESS);
  s21_remove_matrix(&result);
...
START_TEST(test_diff_dim_sub) {
  ...
  ck_assert_int_eq(s21_sub_matrix(&a, &b, &result), OK);
  matrix_t expected = {
      .matrix = (double *[]){(double[]){0.1234562}, (double[]){0.7654313}},
      .rows = 2,
      .columns = 1};
  ck_assert_int_eq(s21_eq_matrix(&result, &expected), SUCCESS);
  s21_remove_matrix(&result);
...
START_TEST(test_big_radix_diff_sub) {
...
START_TEST(test_null_arg) {
  matrix_t elem = {
      ...
      .columns = 1};
  matrix_t result;
  ck_assert_int_eq(s21_sub_matrix(&elem, NULL, &result), ERROR);
  ck_assert_int_eq(s21_sub_matrix(NULL, &elem, &result), ERROR);
  ck_assert_int_eq(s21_sub_matrix(&elem, &elem, NULL), ERROR);
}
END_TEST
...
START_TEST(test_null_matrix) {
  ...
  matrix_t result;
  ck_assert_int_eq(s21_sub_matrix(&elem, &null_elem, &result), ERROR);
  ck_assert_int_eq(s21_sub_matrix(&null_elem, &elem, &result), ERROR);
}
END_TEST
...
START_TEST(test_null_row) {
  ...
  matrix_t elem_null_row = {.matrix = (double *[]){(double[]){0.1234567}, NULL},
                            ...
                            .columns = 1};
  matrix_t result;
  ck_assert_int_eq(s21_sub_matrix(&elem, &elem_null_row, &result), ERROR);
  ck_assert_int_eq(s21_sub_matrix(&elem_null_row, &elem, &result), ERROR);
}
END_TEST
...
START_TEST(test_input_matrix_mismatch) {
  ...
  matrix_t result;
  ck_assert_int_eq(s21_sub_matrix(&a, &b, &result), CALC_ERR);
}
END_TEST
...
Suite *s21_sub_matrix_suite() {
  ...
  tcase_add_test(positive_case, test_simple_sub);
  tcase_add_test(positive_case, test_zero_sub);
  tcase_add_test(positive_case, test_complex_sub);
  tcase_add_test(positive_case, test_diff_dim_sub);
  tcase_add_test(positive_case, test_big_radix_diff_sub);
